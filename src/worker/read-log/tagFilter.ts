...
// 开始攻击命名怪的正则
const firstAttackRegex = new RegExp(`对(${bosses.map((boss) => boss.name).join("|")})造成了${"(.+?)"}的伤害`);
// 命名怪倒地的正则
const bossDeadRegex = new RegExp(`从(${bosses.map((boss) => boss.name).join("|")})获得了${"(.+?)"}的经验值`);
// 整点时间正则
...
export function tagFilter(
  ...
  if (logContent.match(mapChangeRegex)) {
    ...
    tagList.push(tag);
    matched = true;
  } else if (firstAttackRegex.test(logContent) && !usedBosses.includes(logContent.match(firstAttackRegex)![1])) {
    /*
     * Tag2 开始攻击命名怪
    ...
    matched = true;
  } else if (logContent.includes("龙族保护魔法阵已发动。")) {
    /*
     * Tag2 进入神将房
     * eg: 龙族保护魔法阵已发动。
     */
